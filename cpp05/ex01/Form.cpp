...
int Form::ft_strlen(std::string name) const{
	int i = 0;
	while (name[i]) {
		i++;
	}
...
Form::Form(std::string name, int grade, int exec) : _name(name), _signed(false), _grade(grade), _exec(exec)
{
	...
	if (ft_strlen(this->_name) < 1)
		throw Form::EmptyLine(); 
	else if (this->_grade < HIGH || this->_exec < HIGH)
...
const char *Form::EmptyLine::what() const throw(){
	return "Form: Line is empty";
}
